// Right rail — Ask Yukti RAG chat panel.

const { useState: useStateAY, useRef: useRefAY, useEffect: useEffectAY } = React;

function AskYukti({ context, pendingPrompt, onPromptConsumed }) {
  const [messages, setMessages] = useStateAY([]);
  const [input, setInput] = useStateAY("");
  const [busy, setBusy] = useStateAY(false);
  const bodyRef = useRefAY(null);
  ...
  const sendMessage = (text) => {
    ...
    setBusy(true);
    setInput("");
    // Mock RAG response — picks plausible cites from registry.
    setTimeout(() => {
    ...
    <aside className="rail">
      ...
      <div className="rail__ctx">
        ...
        <span className="rail__ctx-val">
          {(context.gaWeeks != null || context.gaDays != null) ? `GA ${context.gaWeeks ?? 0}w${context.gaDays ?? 0}d` : "GA —"}
          {" · "}
          {context.rhStatus ? `Rh ${context.rhStatus[0].toUpperCase()}` : "Rh —"}
          {" · "}
          {context.hemoStatus || "stable"}
        </span>
      </div>
            ...
            <div key={i} className="msg msg--bot">
              ...
              {m.sources && m.sources.length > 0 && (
                <div className="msg__sources">
                  {m.sources.map(srcId => {
                    const idx = window.YK_REFS.findIndex(r => r.id === srcId);
                    if (idx < 0) return null;
                    const ref = window.YK_REFS[idx];
                    return (
                      <a
                        key={srcId}
                        href={`#ref-${srcId}`}
...
// Tiny mock answer generator — wires citations to ref ids.
function mockAnswer(prompt, ctx) {
  const p = prompt.toLowerCase();
  if (p.includes("rhogam") || p.includes("rh ")) {
    return {
      text: `For an Rh(D)-negative patient with first-trimester bleeding or confirmed EPL, Memorial Regional stocks 300mcg IM RhoGAM (the 50mcg mini-dose was discontinued in 2024). Administer within 72 hours of bleeding onset. SMFM 2024 supports this dosing for any EPL ≥7 weeks GA.`,
  ...
  return {
    text: `Based on the active pathway: ${(ctx.gaWeeks != null || ctx.gaDays != null) ? `${ctx.gaWeeks ?? 0}w${ctx.gaDays ?? 0}d GA` : "GA pending"}, ${ctx.rhStatus || "Rh status unknown"}, ${ctx.hemoStatus || "stable"}. I can compare management options, walk through SRU criteria, surface the local RhoGAM protocol, or pull discharge red flags. Try one of the suggested questions.`,
    sources: ["acog-200-2018"],
  };
}

window.AskYukti = AskYukti;
